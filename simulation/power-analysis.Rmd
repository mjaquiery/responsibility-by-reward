...
data <- params %>%
  mutate(
    result = pmap(.l = ., .f = ~ generateData(..1, ..2, ..2 / 3, ..3, ..3 / 3)),
    run = paste0('n = ', n, '; bes = ', beneficiary_es, '; bes_sd = ',
                  beneficiary_es / 3, '; es = ', es, '; es_sd = ', es / 3, 
                  '; rep = ', rep)
  )
...
data %>% group_by(pid, run, outcome, pGetsOutcome) %>%
  rename(b_es = beneficiary_es) %>%
  dplyr::filter(n == max(n)) %>%
  summarise_if(is.numeric, mean) %>%
  ggplot(aes(x = pGetsOutcome, y = scaleResp, colour = outcome)) +
  stat_summary(geom = "line", aes(group = paste(outcome)), 
               fun.y = mean, position = position_dodge(.0)) +
  stat_summary(geom = 'errorbar', aes(group = paste(outcome)), width = 0,
...
suppressWarnings(
  aovs <- data %>%
    mutate_at(vars(pid, outcome, pGetsOutcome), factor) %>%
    nest(data = c(-run, -es, -beneficiary_es, -rep, -n)) %>%
...
aovs %>% group_by(es, beneficiary_es, n) %>%
...
Because we have no effect of participant getting the outcome where effectSize = 0, the pSig represents a false positive rate. Anywhere else, we know there is an effect, so (1 - pSig) gives the false negative rate. Note that stronger main effects of beneficiary reduce the power to detect the interaction, so having a reasonable estimate of the size of this effect will help for calculating power.

### Next steps

We now have the tools to calculate power empirically. 
