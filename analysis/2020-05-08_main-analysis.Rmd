...
Here we would like to address 1) whether outcome ownership changes attributions of responsibility and 2) whether outcome ownership is necessary for the self-serving and/or other-serving bias to appear.  
...
Participants will perform an online task where they make collective decisions through majority votes, then one member of the group can receive either a reward or no reward. 
Finally they will rate the responsibility of the group members for the received positive or negative outcome. 
This paradigm will allow us to address both questions stated above: 

1. by investigating whether responsibility attribution increases when a group member receives the outcome versus does not receive the outcome, although the control over the outcome is exactly similar. 

2. By checking whether the self and other-serving bias of higher responsibility ratings for positive outcomes depend on who gets the outcome in a group decision. 
...
prettify <- function(d)
  ...
  mutate(`Gamble outcome recipient` = 
         ...
         `Gamble outcome` = if_else(outcome == 'reward', "Reward", "No reward"),
         `Gamble vote result` = if_else(status == 'majority', 
...
### 1) Do participants feel more responsible if they receive the outcome versus do not receive the outcome?
...
tmp <- d %>% 
  prettify() %>%
  group_by(subject_id, `Gamble outcome recipient`) %>%
  filter(!is.na(response)) %>%
  summarise(response = mean(response), .groups = 'drop') 

bf <- ttestBF(
  x = tmp$response[tmp$`Gamble outcome recipient` == 'Another player'],
  y = tmp$response[tmp$`Gamble outcome recipient` != 'Another player'],
  paired = T
)

bf <- exp(bf@bayesFactor$bf) %>% round(2)

ggplot(tmp, aes(x = `Gamble outcome recipient`, y = response)) +
  geom_line(aes(group = subject_id), colour = 'grey') +
  geom_boxplot(outlier.colour = NA, width = dw,
               aes(x = nudge(`Gamble outcome recipient`, dw),
                   group = `Gamble outcome recipient`)) +
...
### 2) Does the self-serving bias, i.e, higher responsibility for positive vs negative outcomes, change if another member of their group receives the outcome?

```{r interaction of outcome valance and getting the outcome}

tmp <- d %>% 
  prettify() %>%
  group_by(subject_id, `Gamble outcome recipient`, `Gamble outcome`) %>%
  filter(!is.na(response)) %>%
  summarise(response = mean(response), .groups = 'drop')

bf <- tmp %>%
  nest(d = -`Gamble outcome recipient`) %>%
  mutate(
    bf = map(d, ~ ttestBF(
      x = .$response[.$`Gamble outcome` == 'No reward'],
      y = .$response[.$`Gamble outcome` != 'No reward'],
      paired = T
    )),
...
ggplot(tmp, aes(x = `Gamble outcome`, y = response)) +
  geom_line(aes(group = subject_id), colour = "grey") +
  geom_boxplot(outlier.colour = NA, width = dw,
               aes(x = nudge(`Gamble outcome`, dw),
                   group = `Gamble outcome`)) +
  geom_segment(x = 1 - dw, xend = 2 + dw, y = 101, yend = 101) +
  geom_label(aes(label = paste0('BF = ', bf)), x = 1.5, y = 101, data = bf) +
  scale_x_discrete(expand = c(0, dw * 4)) +
  scale_y_continuous(limits = c(0, 100)) +
  labs(y = 'Participant rating of own responsibility') +
  facet_wrap(~paste0(`Gamble outcome recipient`, ' gets outcome'))
...
d %>%
  prettify() %>%
  group_by(subject_id, `Gamble vote result`, `Gamble outcome`, `Gamble outcome recipient`) %>% 
  filter(!is.na(response)) %>%
  summarise(response = mean(response), .groups = 'drop') %>%
  ggplot(aes(x = `Gamble outcome`, y = response, colour = `Gamble outcome recipient`)) +
  geom_hline(yintercept = 20, linetype = 'dashed') +
  geom_point(alpha = .1, position = position_jitterdodge(dw/1.5, dodge.width = dw)) +
  stat_summary(geom = "point", aes(group = `Gamble outcome recipient`), size = 4, 
...
Participants showed a strong tendency to provide higher ratings for reward versus non-reward outcomes ($F$(1, 53) = 28.46, $p$ < .001, $\eta_p^2$ = .030) and if they rather than another participant received the outcome ($F$(1, 53) = 15.48, $p$ < .001, $\eta_p^2$ = .017). 
There was no main effect of whether the participant was in the Majority or Minority for the choice of gamble ($\text{BF}_{10}$ = 1/25.09).
Moreover, the increase in ratings for rewarded vs non-rewarded trials were greater where the Participant as opposed to Another player received the outcome ($F$(1, 53) = 9.46, $p$ < .001, $\eta_p^2$ = .005) or where the participant was in the Majority as opposed to the Minority for the choice of gamble ($F$(1, 53) = 5.29, $p$ = .025, $\eta_p^2$ = .012). 
The increase in ratings of responsibility for trials on which Participant as opposed to Another player was equivalent whether the participant was in the Majority or Minority for the choice of gamble ($\text{BF}_{10}$ = 1/16.81).
Finally, there was a three-way interaction: the difference in responsibility for rewarded vs non-rewarded trials according to whether the participant received the outcome differed between the participants' minority and majority trials ($F$(1, 53) = 7.64, $p$ = .001, $\eta_p^2$ = .003), although this was not confirmed by the Bayesian statistics because the result was uninformative (BF = 1.55).
