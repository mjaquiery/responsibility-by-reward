...
Before exclusion we have `r length(unique(d.old$subject_id))` subject_ids. We now exclude participants who have more than `r max_missed_trials` missed trials, more than `r max_consec_resp` consecutive left or right responses (after removing incomplete responses), or fewer than `r expected_trial_n` trials recorded. Our final participant list contains `r length(unique(d$subject_id))` participants.
...
a.bf %>% 
  ...
  mutate(`.3<BF>3` = case_when(BF.M1.M2 < 1/3 ~ '-', 
                                   BF.M1.M2 > 3 ~ '+', 
                                   T ~ '')) %>%
  apa_table()
...
We had three main hypotheses we wish to investigate:

**1. Does responsibility attribution increase when a group member receives the outcome versus does not receive the outcome?**
This is revealed in the main effect of whether the participant receives the outcome ('getsout'). 
We see the expected main effect in the expected direction.

**1b. Is the responsibility increase from receiving the outcome stronger when the participant vs another player receives the outcome?** 
This pilot is not set up to answer this question because there are not attributions of responsibility for the receiving player when that player is not the participant.

**2. Will the self and other-serving bias of higher responsibility ratings for positive outcomes depend on who gets the outcome?**
This is revealed in the interaction between the effects of whether the participant receives the outcome ('getsout') and whether the outcome was good ('outcome'). 
We see the expected interaction in the expected direction.

# Discussion

Pilot experiment 1 suggested that participants' feelings of responsibility were increased where the participant received the outcome of the chosen gamble (whether good or neutral). 
This increase was greater where the outcome was good rather than neutral.
