...
Data for this pilot experiment are stored in two separate .csv files on GitHub.
The first 50 participants were recruited initially, then the remaining participants were recruited after investigating the data using Bayesian tests.  
* [First 50 participants](https://raw.githubusercontent.com/mjaquiery/responsibility-by-reward/master/data/data50part.csv)  
* [Remaining participants](https://raw.githubusercontent.com/mjaquiery/responsibility-by-reward/master/data/dataextra.csv)
...
suppressWarnings({
  fNames <- c('../data/data50part.csv', '../data/dataextra.csv')
  ...
  x <- read.csv(fNames[1], stringsAsFactors = F) %>% 
    mutate_at(vars(c(trialnum, outcome)), as.numeric)
  y <- read.csv(fNames[2], stringsAsFactors = F)
})
...
The figure shows participant mean self-responsibility ratings collapsed across other trial factors (lines) and their distributions (box plots).
The effect of 'self-serving bias' seems more pronounced on trials where the participant gets the outcome than on trials where another player gets the outcome. 


We explored this statistically with a 2x2x2 ANOVA in which we saw an interaction between the effects of whether the participant receives the outcome ('getsout') and whether the outcome was a Reward ('outcome'). 
