...
jsPsych.plugins['html-slider-response2'] = (function() {

  var plugin = {};

  plugin.info = {
    name: 'html-slider-response2',
    description: '',
    parameters: {
      stimulus: {
  ...
  plugin.trial = function(display_element, trial) {
    ...
    for(var j=0; j < trial.labels.length; j++){
      ...
      html += '<span style="text-align: center; font-size: 100%;">'+trial.labels[j]+'</span>'; //Marwa increased text
      html += '</div>'
    }
    html += '</div>';
    html += '</div>';
    ...
    display_element.querySelector('#jspsych-html-slider-response-next').addEventListener('click', function() {
      // measure response time
       ...
       //Marwa added code below to restrict answering 50
var respscale=display_element.querySelector('#jspsych-html-slider-response-response').value;

  //if (respscale != 50) {
      if (respscale < 48 | respscale > 52 ) {
